      ...
      <v-data-table :items="items" :headers="headers">
        ...
        <template #[`item.quantity`]="{ item }">
          <v-btn
            variant="text"
            color="red"
            @click="addCart_P(item.p_id._id, -1)"
            >-</v-btn
          >
        ...
        <template #[`item.action`]="{ item }">
          <v-btn
            ...
            color="red"
            icon="mdi-delete"
            @click="addCart_P(item.p_id._id, item.quantity * -1)"
          ></v-btn>
        </template>
      </v-data-table>
    </v-col>
...
const loadItems = async () => {
  try {
    const { data } = await apiAuth.get("/user/cart_P");
...
const checkout = async () => {
  ...
  const result = await user.checkout_P();
